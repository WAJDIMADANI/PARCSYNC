...
export function IncidentsList({ onViewProfile }: IncidentsListProps = {}) {
  ...
  const fetchIncidents = async () => {
    try {
      // Utiliser la vue v_gestion_incidents_source qui applique déjà tous les filtres :
      // - statut = 'expire'
      // - date_expiration_originale < today
      // - exclusion des profils sortis (date_sortie IS NULL)
      ...
      // Formatter les données pour correspondre à l'interface Incident
      ...
      // Vérifier que toutes les dates sont bien dans le passé
      const today = new Date();
      today.setHours(0, 0, 0, 0);

      const validIncidents = formattedIncidents.filter(incident => {
        const expDate = new Date(incident.date_expiration_originale);
      ...
      console.log('📊 Incidents expirés depuis v_gestion_incidents_source:', {
        total: validIncidents.length,
        titre_sejour: validIncidents.filter(i => i.type === 'titre_sejour').length,
        visite_medicale: validIncidents.filter(i => i.type === 'visite_medicale').length,
        permis_conduire: validIncidents.filter(i => i.type === 'permis_conduire').length,
        contrat_cdd: validIncidents.filter(i => i.type === 'contrat_expire' && i.metadata?.contrat_type === 'cdd').length,
        avenant: validIncidents.filter(i => i.type === 'contrat_expire' && i.metadata?.contrat_type === 'avenant').length
      });
    } catch (error) {
  ...
  const getTabCount = (type: string) => {
    if (type === 'contrat_cdd') {
      return incidents.filter(i => {
        ...
        // La vue filtre déjà sur statut='expire', pas besoin de filtrer à nouveau
    ...
    // La vue retourne déjà uniquement les incidents expirés
  ...
  const filteredIncidents = incidents.filter(incident => {
    ...
    // La vue v_gestion_incidents_source retourne déjà uniquement les incidents avec statut='expire'
    // Donc pas besoin de filtrer sur le statut ici
    // Note: filterStatus est conservé pour compatibilité UI mais n'a plus d'effet réel
