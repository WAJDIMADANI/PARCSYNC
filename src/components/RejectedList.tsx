...
export function RejectedList() {
  ...
  const [editingCodeCandidate, setEditingCodeCandidate] = useState<RejectedCandidate | null>(null);
  ...
  const fetchData = async () => {
    try {
      const { data: candidatsData, error: candidatsError } = await supabase
        .from('candidat')
        .select('*')
        .eq('statut_candidature', 'candidature_rejetee')
        .is('deleted_at', null)
        .order('created_at', { ascending: false });

      if (candidatsError) throw candidatsError;

      const rejectedData = candidatsData?.map(c => ({
        candidat_id: c.id,
        candidat_nom: c.nom,
        ...
        statut_candidature: c.statut_candidature || 'candidature_rejetee',
      ...
      setCandidates(rejectedData);
      setAvailableDepartements(depts as string[]);
      setSites(sitesRes.data || []);
      setSecteurs(secteursRes.data || []);
      setPostes(postesRes.data || []);
    } catch (error) {
      console.error('Erreur chargement candidatures rejetées:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatDisponibilite = (candidate: RejectedCandidate): string => {
  ...
  const getDisponibiliteSortValue = (candidate: RejectedCandidate): number => {
    if (candidate.date_disponibilite) {
  ...
  const openCodeCouleurModal = (candidate: RejectedCandidate) => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <LoadingSpinner size="lg" text="Chargement des candidatures rejetées..." />
      </div>
    );
  }
    ...
    <div>
      <div className="mb-6 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">{candidates.length} candidature(s) rejetée(s)</p>
        </div>
      </div>
      ...
      {filteredCandidates.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          <p className="text-gray-600">Aucune candidature rejetée</p>
        </div>
      ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-2" onClick={(e) => e.stopPropagation()}>
                      <select
                        value={candidate.statut_candidature || 'candidature_rejetee'}
                        onChange={async (e) => {
                          try {
                            await supabase
                              .from('candidat')
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-gray-900">
            {isViewMode ? 'Fiche candidat - Rejeté' : 'Modifier le candidat'}
          </h2>
