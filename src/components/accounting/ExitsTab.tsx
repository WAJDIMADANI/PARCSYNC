...
export default function ExitsTab() {
  ...
  const fetchExits = async () => {
    ...
    try {
      ...
      if (dateDebut) {
        query = query.gte('date_sortie', dateDebut);
      }
      if (dateFin) {
        query = query.lte('date_sortie', dateFin);
      }

      const { data, error } = await query.order('date_sortie', { ascending: false });
  ...
  const exportToExcel = () => {
    ...
    const data = employees.map(emp => ({
      ...
      'Poste': emp.poste,
      'Site': emp.site,
      'Date de départ': emp.date_sortie,
      'Motif du départ': emp.motif_depart,
      'Commentaires': emp.commentaire_depart || '',
    }));
    ...
    const fileName = `Sorties_${dateDebut}_${dateFin}.xlsx`;
    ...
    <div className="px-0 py-0">
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-gray-900">Sorties de salariés</h2>
          <button
      ...
      {employees.length > 0 ? (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Date de départ</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Motif</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Commentaires</th>
                </tr>
              </thead>
                ...
                {employees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getMotifColor(emp.motif_depart)}`}>
                        {emp.motif_depart}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600 max-w-xs truncate" title={emp.commentaire_depart || ''}>
        ...
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          <p className="text-gray-500">
            {loading ? 'Chargement...' : 'Sélectionnez une plage de dates et cliquez sur "Rechercher" pour voir les sorties'}
          </p>
        </div>
      )}
    </div>
