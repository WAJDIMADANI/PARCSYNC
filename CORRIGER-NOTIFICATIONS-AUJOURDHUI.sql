...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE table_name = 'notification' AND column_name = 'titre'
  ) INTO v_has_titre_column;
  ...
  ELSE
    -- Créer la notification (avec ou sans titre/message selon la structure)
    IF v_has_titre_column THEN
...
SELECT
  ...
  n.metadata->>'matricule' as "Matricule"
FROM notification n
