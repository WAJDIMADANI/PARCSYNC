/*
  # Fix: Incidents contrats expirés obsolètes quand CDI existe

  ## Problème
  Des profils avec CDI signé/actif (date_fin NULL) apparaissent encore dans "Contrats expirés"
  à cause de leurs anciens CDD/avenants expirés.

  Exemple: Didier RENARD a un CDI signé mais des incidents contrat_expire existent sur ses anciens avenants.
  ...
  3. Ce CDI commence le jour même ou après la fin du CDD/avenant expiré (couverture)

  ## Solution
  1. Modifier get_cdd_expires() pour vérifier la couverture CDI
  2. Modifier get_avenants_expires() pour vérifier la couverture CDI
  3. Résoudre automatiquement les incidents obsolètes existants
  4. Ajouter un trigger pour auto-résoudre quand un CDI devient signé/actif
*/
...
BEGIN
  ...
  profils_avec_cdi_couvrant AS (
    -- Profils ayant un CDI signé/actif qui COUVRE la période après le CDD
    ...
    WHERE
      ...
      AND cdi.date_fin IS NULL  -- CDI ouvert
      AND cdi.date_debut >= cdd.date_expiration_reelle  -- CDI commence le jour même ou après l'expiration
  )
  SELECT
    cdd.profil_id,
    cdd.nom,
  ...
  FROM cdd_avec_date_reelle cdd
  WHERE NOT EXISTS (
    -- Exclure si un CDI couvrant existe
    SELECT 1
...
COMMENT ON FUNCTION get_cdd_expires() IS 'Retourne les CDD expirés/à expirer, SAUF si un CDI couvrant existe';
...
BEGIN
  ...
  profils_avec_cdi_couvrant AS (
    -- Profils ayant un CDI signé/actif qui COUVRE la période après l'avenant
    ...
    WHERE
      ...
      AND cdi.statut IN ('actif', 'signed', 'signe')
      AND cdi.date_fin IS NULL  -- CDI ouvert
      AND cdi.date_debut >= av.date_expiration_reelle  -- CDI commence le jour même ou après l'expiration
  )
  SELECT
    av.profil_id,
    av.nom,
...
COMMENT ON FUNCTION get_avenants_expires() IS 'Retourne les avenants expirés, SAUF si un CDI couvrant existe';


-- ========================================
-- ÉTAPE 3: Fonction pour résoudre les incidents obsolètes
-- ========================================
...
GRANT EXECUTE ON FUNCTION resoudre_incidents_contrats_obsoletes TO authenticated;

COMMENT ON FUNCTION resoudre_incidents_contrats_obsoletes() IS 'Résout automatiquement les incidents contrat_expire obsolètes car un CDI existe';
...
AS $$
BEGIN
  -- Si un contrat devient un CDI signé/actif avec date_fin NULL
  ...
  THEN
    -- Résoudre tous les incidents contrat_expire du profil couverts par ce CDI
    UPDATE incident
    SET
      statut = 'resolu',
      date_resolution = NOW(),
...
COMMENT ON FUNCTION trigger_resoudre_incidents_obsoletes_sur_cdi() IS 'Auto-résout les incidents contrat_expire quand un CDI est créé/signé';


-- ========================================
-- ÉTAPE 5: BACKFILL - Résoudre les incidents existants
-- ========================================

SELECT '=== Résolution des incidents obsolètes existants ===' as etape;
...
-- Profils avec CDI qui n'ont plus d'incidents actifs
SELECT
  'Profils avec CDI sans incidents actifs' as info,
  COUNT(DISTINCT p.id) as nombre
FROM profil p
INNER JOIN contrat cdi ON cdi.profil_id = p.id
WHERE LOWER(cdi.type) = 'cdi'
  AND cdi.statut IN ('actif', 'signed', 'signe')
  AND cdi.date_fin IS NULL
  AND NOT EXISTS (
    SELECT 1
...
SELECT
  'Fonction get_cdd_expires()' as fonction,
  COUNT(*) as nb_resultats
FROM get_cdd_expires();

SELECT
  'Fonction get_avenants_expires()' as fonction,
  COUNT(*) as nb_resultats
FROM get_avenants_expires();

-- Test spécifique pour Didier RENARD (exemple cité)
SELECT
  p.nom,
  p.prenom,
  ...
  COUNT(i.id) as incidents_actifs
FROM profil p
INNER JOIN contrat cdi ON cdi.profil_id = p.id
...
WHERE p.nom ILIKE '%RENARD%'
  ...
  AND LOWER(cdi.type) = 'cdi'
GROUP BY p.id, p.nom, p.prenom, cdi.id, cdi.type, cdi.statut, cdi.date_debut, cdi.date_fin;
